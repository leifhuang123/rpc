...
{
    client_t *this = (client_t *)arg;
    assert(this != NULL);

    return_type ret;
    ret = deserialize(this->buf);

    // Process and send result back to client
    unsigned char *tmp = int_serialize(this->buf, ret.return_size);
    if (ret.return_size > 0)
        memcpy(tmp, ret.return_val, ret.return_size);

    sendto(this->sockfd, this->buf, ret.return_size + sizeof(int), 0, (struct sockaddr *)&(this->addr), sizeof(struct sockaddr_in));
    LOGA("send", this->buf, ret.return_size + sizeof(int));
 ...
 */
void launch_server(int thread_number)
{
    // create a rpc server
    ...
    bool malloc_new_client = true;
    client_t *new_client = NULL;
    int received_size;

    while (1)
    {
        if (malloc_new_client)
        {
        ...
        if (new_client != NULL)
        {
            ...
            received_size = recvfrom(sockfd, new_client->buf, RPC_BUFFER_SIZE,
                                 0, (struct sockaddr *)&new_client->addr, &addr_len);
            
            // distribute task
            if (received_size > 0)
            {
                threadpool_append(pool, worker, new_client);
                malloc_new_client = true;
            }
            else
            {
               malloc_new_client = false;
            }
        }
    }
